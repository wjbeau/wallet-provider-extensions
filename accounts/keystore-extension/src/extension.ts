...
import type { AccountsKeystoreExtension, AccountsKeystoreExtensionOptions } from "./types.ts";
    ...
    for (const key of keys) {
      ...
      if (key.type === "hd-derived-ed25519") {
        provider.account.store.addAccount(
          createKeyAccount(
            key.id,
            ((key as XHDDerivedKeyData)?.metadata?.address?.algorand as string) ??
              "TODO: Add addresses to types",
          ),
        );
      }
    }
