    ...
    key: {
      store: options?.api?.keystore || {
        ...
        generate: (options): Promise<KeyId> => {
          ...
          if (requiresParentKey(options) && typeof options?.params?.parentKeyId === "undefined") {
            throw new InvalidKeyDataError("Parent key ID is required for generating derived keys");
          }
                ...
                if (
                  ...
                  if (!parentKey)
                    throw new KeyNotFoundError(
                      (key as XHDDerivedKeyData | XHDDomainP256KeyData).metadata.parentKeyId,
                    );
        ...
        decryptWithKey: (id, data, algorithm): Promise<Uint8Array<ArrayBufferLike>> =>
          keyStoreHooks(
            "decryptWithKey",
            async () => {
              let key: KeyData | null = null;
        ...
        batchSign: (ids, data): Promise<Uint8Array<ArrayBufferLike>[]> =>
          keyStoreHooks(
            "batchSign",
            async ({ ids, data }: { ids: KeyId[]; data: Uint8Array<ArrayBufferLike>[] }) => {
